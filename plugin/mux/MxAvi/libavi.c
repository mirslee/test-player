...
static MxFourcc GetFOURCC( const uint8_t *p_buff )
{
    return MX_FOURCC( p_buff[0], p_buff[1], p_buff[2], p_buff[3] );
}
...
static int AVI_ChunkRead_strf( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    if( p_chk->common.p_father == NULL )
    {
        ...
        AVI_READCHUNK_EXIT( MX_EGENERIC );
    }
    if( !( p_strh = AVI_ChunkFind( p_chk->common.p_father, AVIFOURCC_strh, 0, false ) ) )
    {
        msg_Err( (vlc_object_t*)s, "malformed avi file" );
        AVI_READCHUNK_EXIT( p_chk->common.i_chunk_size > 0  ? MX_EGENERIC : AVI_ZEROSIZED_CHUNK );
    }
...
static int AVI_ChunkRead_strd( stream_t *s, avi_chunk_t *p_chk )
{
    ...
    AVI_READCHUNK_EXIT( p_chk->strd.p_data ? MX_SUCCESS : MX_EGENERIC );
}
...
static int AVI_ChunkFunctionFind( MxFourcc i_fourcc )
{
...
int  AVI_ChunkRead( stream_t *s, avi_chunk_t *p_chk, avi_chunk_t *p_father )
{
    ...
    if( p_chk->common.i_chunk_fourcc == MX_FOURCC( 0, 0, 0, 0 ) )
    {
...
int AVI_ChunkReadRoot( stream_t *s, avi_chunk_t *p_root )
{
    ...
    p_list->i_type = MX_FOURCC( 'r', 'o', 'o', 't' );
    ...
    for( ; ; )
    {
        ...
        if( AVI_ChunkRead( s, p_chk, p_root ) != MX_SUCCESS )
        {
            AVI_ChunkClean( s, p_chk );
            free( p_chk );
            break;
...
int  AVI_ChunkCount_( avi_chunk_t *p_chk, MxFourcc i_fourcc, bool b_list )
{
    if( !p_chk )
        return 0;
...
void *AVI_ChunkFind_( avi_chunk_t *p_chk,
                      MxFourcc i_fourcc, int i_number, bool b_list )
{
    if( !p_chk )
        return NULL;
