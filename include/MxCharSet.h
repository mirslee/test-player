...
*/
MX_USED static inline const char *IsUTF8(const char *str)
{
	size_t n;
	uint32_t cp;
...
MX_USED static inline char *FromLocale(const char *locale)
{
	return locale ? FromCharset((char *)"", locale, strlen(locale)) : NULL;
}

MX_USED static inline char *ToLocale(const char *utf8)
{
	size_t outsize;
	return utf8 ? (char *)ToCharset("", utf8, &outsize) : NULL;
}

MX_USED static inline void LocaleFree(const char *str)
{
	free((char *)str);
}

MX_USED static inline char *FromLocaleDup(const char *locale)
{
	return FromCharset("", locale, strlen(locale));
}

MX_USED static inline char *ToLocaleDup(const char *utf8)
{
	size_t outsize;
