...
static char filename_buffer[MAX_VXPATH];
...
static void read_parameter_info(FILE * input,
  ...
  while(1)
    ...
    if(CHECK_KEYWORD(type_key))
      ...
      info->stringlist_options = (char **)calloc(info->num_stringlist_options,
                                        sizeof(char*));
      info->stringlist_labels = (char **)calloc(info->num_stringlist_options,
                                       sizeof(char*));
      }
  ...
  while(1)
    ...
    /* Number of Fourccs */
      ...
      if(codec->num_fourccs)
        {
        codec->fourccs = (char **)malloc(codec->num_fourccs * sizeof(char*));
        for(i = 0; i < codec->num_fourccs; i++)
          codec->fourccs[i] = (char *)malloc(5 * sizeof(char));
        }
      }
