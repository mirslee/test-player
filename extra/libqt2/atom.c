	...
	const char *text)
{
	...
	if(!result) result = !quicktime_write_char32(file, text);
	*(uint32_t*)atom->type = *(uint32_t*)text;
	atom->use_64 = 0;
	return result;
}
...
void quicktime_atom_write_footer(quicktime_t *file, quicktime_atom_t *atom)
{
	...
	{
		if((atom->end-atom->start)>0x7FFFFFFF)
		{
			int64_t newstart = atom->start-8;
			quicktime_set_position(file,newstart);
			quicktime_write_int32(file,1);
			quicktime_write_char32(file,(const char*)atom->type);
			quicktime_write_int64(file,atom->end -newstart);
			atom->start = newstart;
		}
		else
		{
			quicktime_set_position(file, atom->start);
			quicktime_write_int32(file, (long)(atom->end - atom->start));
		}
	}
