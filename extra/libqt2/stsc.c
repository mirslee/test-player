...
void quicktime_stsc_dump(quicktime_stsc_t *stsc)
{
	...
	for(i = 0; i < stsc->total_entries-1; i++)
	{
		lqt_dump("       begin %10I64d chunk %5ld samples %ld id %ld\n", 
			totalsamples,stsc->table[i].chunk, stsc->table[i].samples, stsc->table[i].id);
		int chunk1 = stsc->table[i].chunk;
		int chunk2 = stsc->table[i+1].chunk;
		__int64 samples = stsc->table[i].samples;
		totalsamples += samples*(chunk2-chunk1);
	}
	lqt_dump("       begin %10I64d chunk %5ld samples %ld id %ld\n", 
		totalsamples,stsc->table[stsc->total_entries-1].chunk, stsc->table[stsc->total_entries-1].samples, stsc->table[stsc->total_entries-1].id);
}
