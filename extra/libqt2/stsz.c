...
void quicktime_stsz_dump(quicktime_stsz_t *stsz)
{
	...
	lqt_dump("      version %d\n", stsz->version);
	lqt_dump("      flags %ld\n", stsz->flags);
	lqt_dump("      sample_size %ld\n", stsz->sample_size);
	lqt_dump("      total_entries %u\n",(uint32_t)stsz->total_entries);
	
        if(!stsz->sample_size)
          {
          for(i = 0; i < stsz->total_entries; i++)
            {
            lqt_dump("       sample_size 0x%x (%d)\n",
                     (int)(stsz->table[i].size), (int)(stsz->table[i].size));
            }
          }
}
...
// relies on the samples per chunk table to determine the chunk size.
	quicktime_write_int32(file, (int)stsz->sample_size);
	...
	if(!stsz->sample_size)
	{
		for(i = 0; i < stsz->total_entries; i++)
		{
			quicktime_write_int32(file, (int)stsz->table[i].size);
		}
	}
