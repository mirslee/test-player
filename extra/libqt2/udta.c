...
static const char copyright_id[] = { (char)0xa9, 'c', 'p', 'y' };
static const char info_id[]		 = { (char)0xa9, 'i', 'n', 'f' };
static const char name_id[]		 = { (char)0xa9, 'n', 'a', 'm' };
static const char artist_id[]	 = { (char)0xa9, 'A', 'R', 'T' };
static const char album_id[]	 = { (char)0xa9, 'a', 'l', 'b' };
static const char track_id[]	 = { (char)0xa9, 't', 'r', 'k' };
static const char comment_id[]	 = { (char)0xa9, 'c', 'm', 't' };
static const char author_id[]	 = { (char)0xa9, 'a', 'u', 't' };
static const char genre_id[]	 = { (char)0xa9, 'g', 'e', 'n' };
static const char trkn_id[]      = {        't', 'r', 'k', 'n' };
...
int quicktime_read_udta_string(quicktime_t *file,
  ...
  if(*size){ free(*string); *string = NULL;}
  if(!ilst)
    {
    ...
    *size = (int)(leaf_atom.end - quicktime_position(file));  /* Size of string */
    *string = (char*)malloc(*size + 1);
    result = quicktime_read_data(file, (uint8_t*)(*string), *size);
    (*string)[*size] = 0;
    return !result;
...
int quicktime_read_udta(quicktime_t *file, quicktime_udta_t *udta,
  ...
  while(udta_atom->end - quicktime_position(file) >= 8)
    ...
    if(quicktime_atom_is(&leaf_atom, "meta"))
      ...
      udta->track_len = 6;
      udta->track = (char*)malloc(6); /* strlen("65535") + 1 */
...
quicktime_write_udta_string(quicktime_t *file, const char * str,
  ...
  if(!ilst)
    ...
    result = quicktime_write_data(file, (const uint8_t*)(str), (int)strlen(str));
