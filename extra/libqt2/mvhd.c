...
void quicktime_read_mvhd(quicktime_t *file, quicktime_mvhd_t *mvhd, quicktime_atom_t *parent_atom)
{
	...
	if(mvhd->version == 0)
	{
		mvhd->creation_time = quicktime_read_uint32(file);
		mvhd->modification_time = quicktime_read_uint32(file);
	}
...
void quicktime_write_mvhd(quicktime_t *file, quicktime_mvhd_t *mvhd)
{
	...
	if(mvhd->version == 0)
	{
		quicktime_write_int32(file, (long)mvhd->creation_time);
		quicktime_write_int32(file, (long)mvhd->modification_time);
	}
	else if(mvhd->version == 1)
	{
	...
	if(mvhd->version == 0)
		quicktime_write_int32(file, (long)mvhd->duration);
	else if(mvhd->version == 1)
