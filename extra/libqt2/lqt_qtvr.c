...
{
    return (int)(((hdeg * fabs(vdeg-90)) / 64800) * 
...
void lqt_qtvr_get_pan(quicktime_t  *file, float *minpan, float *maxpan, float *defpan)
{
    if (lqt_qtvr_get_object_track(file) >= 0) {
        ...
        if (defpan != NULL ) *defpan = file->qtvr_node[0].obji.defaultPan;
    }
    else {
...
void lqt_qtvr_get_tilt(quicktime_t  *file, float *mintilt, float *maxtilt, float *deftilt)
{
    if (lqt_qtvr_get_object_track(file) >= 0) {
        ...
        if (deftilt != NULL ) *deftilt = file->qtvr_node[0].obji.defaultTilt;
    }
    else {
...
{
    if (lqt_is_qtvr(file) == QTVR_OBJ) {
        file->moov.trak[lqt_qtvr_get_qtvr_track(file)]->tkhd.track_height = (float)height;
        file->moov.trak[lqt_qtvr_get_object_track(file)]->tkhd.track_height = (float)height;
    }
    else if (lqt_is_qtvr(file) == QTVR_PAN) {
        file->moov.trak[lqt_qtvr_get_panorama_track(file)]->tkhd.track_height = (float)height;
    }
