...
void quicktime_stts_init_table(quicktime_stts_t *stts)
  ...
  if(!stts->total_entries)
    ...
    stts->table = (quicktime_stts_table_t *)calloc(stts->total_entries, sizeof(*stts->table));
    }
  }
...
void quicktime_read_stts(quicktime_t *file, quicktime_stts_t *stts)
  ...
  stts->table = (quicktime_stts_table_t *)malloc(sizeof(*stts->table) * stts->total_entries);
  for(i = 0; i < stts->total_entries; i++)
    {
...
{
	int total_entries = stts->total_entries;
	int64_t addsamples = (int64_t)stts->table[total_entries-1].sample_count+sample;
	if(addsamples>INT32_MAX)
	{
		stts->entries_allocated = total_entries+1;
		stts->table = (quicktime_stts_table_t *)realloc(stts->table,
			stts->entries_allocated * sizeof(*(stts->table)));
		stts->table[total_entries].sample_count = 0;
		stts->table[total_entries].sample_duration = stts->table[total_entries-1].sample_duration;
		stts->total_entries++;
		total_entries = stts->total_entries;
	}
	stts->table[total_entries-1].sample_count += sample;
}
	...
	while(sample < stts->total_entries)
	{
		...
		if(stts->table[sample].sample_count > 1)
		{
			...
			if(stts->total_entries - sample - i)
				memmove(&stts->table[sample + 1], &stts->table[sample + i],sizeof(stts->table[sample + i]) * (stts->total_entries - sample - i));
