                        ...
                        quicktime_stbl_t *stbl, quicktime_atom_t *parent_atom)
{
	...
	do
	{
		...
		else if(quicktime_atom_is(&leaf_atom, "stts"))
		{
			...
			if(minf->is_audio&&stbl->stts.total_entries>1)
			{
				__int64 samplecounts = (uint32_t)stbl->stts.table[0].sample_count;
				if(samplecounts>INT32_MAX)
					stbl->stts.table[0].sample_count = (int)(samplecounts-stbl->stts.table[1].sample_count);
			}
		}
		...
		{
			quicktime_read_sdtp(file, &(stbl->sdtp), &leaf_atom);
			stbl->has_sdtp = 1;
		}
