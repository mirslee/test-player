...
void quicktime_read_tkhd(quicktime_t *file, quicktime_tkhd_t *tkhd)
{
	...
	if(tkhd->version == 0)
	{
		tkhd->creation_time = quicktime_read_uint32(file);
		tkhd->modification_time = quicktime_read_uint32(file);
	}
	...
	if(tkhd->version == 0)
		tkhd->duration = quicktime_read_uint32(file);
	else if(tkhd->version == 1)
...
void quicktime_write_tkhd(quicktime_t *file, quicktime_tkhd_t *tkhd)
{
	...
	if(tkhd->version == 0)
	{
		quicktime_write_int32(file, (long)tkhd->creation_time);
		quicktime_write_int32(file, (long)tkhd->modification_time);
	}
	else if(tkhd->version == 1)
	{
    ...
    if(tkhd->version == 0)
        quicktime_write_int32(file, (long)tkhd->duration);
    else if(tkhd->version == 1)
...
void quicktime_tkhd_init_video(quicktime_t *file, 
								...
								int frame_h)
{
	tkhd->track_width = (float)frame_w;
	tkhd->track_height = (float)frame_h;
	tkhd->volume = 0;
}
...
void quicktime_tkhd_init_timecode(quicktime_t *file,
  ...
  tkhd->track_width = (float)frame_w;
  tkhd->track_height = 0;
  tkhd->matrix.values[7] = (float)frame_h;
  tkhd->volume = 0;
  }
