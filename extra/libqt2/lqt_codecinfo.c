...
copy_parameter_info(lqt_parameter_info_t * ret,
  ...
  switch(ret->type)
    ...
    case LQT_PARAMETER_STRINGLIST: /* String with options */
      ...
      ret->stringlist_options = (char**)calloc(ret->num_stringlist_options,
                                       sizeof(char*));
      ret->stringlist_labels = (char**)calloc(ret->num_stringlist_options,
                                      sizeof(char*));
...
copy_codec_info(const lqt_codec_info_t * info)
  {
  int i;
  lqt_codec_info_t * ret = (lqt_codec_info_t*)calloc(1, sizeof(lqt_codec_info_t));
...
//  else
  ...
  if(ret->num_fourccs)
    {
    ret->fourccs = (char**)malloc(ret->num_fourccs * sizeof(char*));
  ...
  if(ret->num_wav_ids)
    {
    ret->wav_ids = (int*)malloc(ret->num_wav_ids * sizeof(int));
  ...
  if(ret->num_encoding_parameters)
    ...
    ret->encoding_parameters =
      (lqt_parameter_info_t*)calloc(ret->num_encoding_parameters+1, sizeof(lqt_parameter_info_t));
  ...
  if(ret->num_decoding_parameters)
    ...
    ret->decoding_parameters =
      (lqt_parameter_info_t*)calloc(ret->num_decoding_parameters, sizeof(lqt_parameter_info_t));
