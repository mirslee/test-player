...
void quicktime_read_stsd_audio(quicktime_t *file, quicktime_stsd_table_t *table,
  ...
  while(quicktime_position(file) < parent_atom->end)
	...
	else if (quicktime_atom_is(&leaf_atom, "glbl"))
	{
		// printf("esds: %"PRId64" bytes\n", leaf_atom.size);
		table->glbl.glbllen = (int)leaf_atom.size-8;
		table->glbl.glbldata = (uint8_t*)malloc(table->glbl.glbllen);
		quicktime_read_data(file,table->glbl.glbldata,table->glbl.glbllen);
		table->has_glbl = 1;
	}
   else
      {
		  ...
		  if (leaf_atom.size > 0)
		  {
			  quicktime_user_atoms_read_atom(file,&table->user_atoms,&leaf_atom);
		  }
      }
	quicktime_atom_skip(file, &leaf_atom);
    } 
...
void quicktime_write_stsd_audio(quicktime_t *file, quicktime_stsd_table_t *table)
  ...
  if(tmp_version < 2)
    ...
    quicktime_write_fixed32(file, (float)table->samplerate);
    if(tmp_version == 1)
      {
...
void quicktime_read_stsd_table_raw(quicktime_t *file, quicktime_stsd_table_t *table)
  ...
  table->table_raw_size = (int)leaf_atom.size;

  table->table_raw = (unsigned char*)malloc(table->table_raw_size);
...
void quicktime_read_stsd_video(quicktime_t *file, quicktime_stsd_table_t *table,
  ...
  if(!table->ctab_id &&
	...
	while(quicktime_position(file) + 8 < parent_atom->end)
    {
		...
		{
			// size = 0 时不应该再进下面的函数
			if ((leaf_atom.size > 0) && (leaf_atom.end <= parent_atom->end))
			{
				quicktime_user_atoms_read_atom(file, &table->user_atoms, &leaf_atom);
			}
			else
			{
				TRACE("leaf_atom 存在问题\n");
			}
		}
		quicktime_atom_skip(file, &leaf_atom);
	}
...
void quicktime_write_stsd_video(quicktime_t *file, quicktime_stsd_table_t *table)
  ...
  int terminate = 0;
  compressor_name_len = (int)strlen(table->compressor_name);
  if(compressor_name_len==0)
  {
	  for(i = 0; i < 32; i++) table->compressor_name[i] = 0;
	  sprintf(table->compressor_name, "%s-%s[%s]", PACKAGE, VERSION,table->format);
  }
  compressor_name_len = (int)strlen(table->compressor_name);
  if(file->file_type & (LQT_FILE_QT|LQT_FILE_QT_OLD))
    {
    ...
    quicktime_write_int32(file, (long)table->data_size);
...
void quicktime_stsd_table_dump(void *minf_ptr, quicktime_stsd_table_t *table)
  {
  quicktime_minf_t *minf = (quicktime_minf_t*)minf_ptr;
