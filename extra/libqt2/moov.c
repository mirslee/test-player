...
/* Read compressed data */
                        int compressed_size = (int)(leaf_atom.end - quicktime_position(file));

                        data_in = (unsigned char*)calloc(1, compressed_size);
                        quicktime_read_data(file, data_in, compressed_size);
/* Decompress to another buffer */
                        data_out = (unsigned char*)calloc(1, uncompressed_size + 0x400);
                        zlib.zalloc = zalloc;
                        zlib.zfree = zfree;
                        zlib.opaque = NULL;
...
void quicktime_finalize_moov(quicktime_t *file, quicktime_moov_t *moov, int64_t suggestduration)
  ...
  for(i = 0; i < moov->total_tracks; i++)
    {
		quicktime_trak_fix_counts(file, moov->trak[i], moov->mvhd.time_scale, suggestduration);
    quicktime_trak_duration(moov->trak[i], &duration, &timescale);
    
    duration = (int64_t)((float)duration / timescale * moov->mvhd.time_scale);
  ...
  if (suggestduration > 0)
	  moov->mvhd.duration = suggestduration;
  else
	  moov->mvhd.duration = longest_duration;
	  if(!IS_MP4(file->file_type))
		moov->mvhd.selection_duration = (long)longest_duration;
  }
