...
void quicktime_ctts_init(quicktime_ctts_t *ctts)
{
	  ...
	  ctts->table = (quicktime_ctts_table_t*)calloc(1, sizeof(quicktime_ctts_table_t) * ctts->entries_allocated);
}

void quicktime_ctts_delete(quicktime_ctts_t *ctts)
{
	if(ctts->entries_allocated) free(ctts->table);
	ctts->total_entries = 0;
...
void quicktime_read_ctts(quicktime_t *file, quicktime_ctts_t *ctts)
  ...
  if(ctts->entries_allocated < ctts->total_entries)
  {
	  ctts->entries_allocated = ctts->total_entries;
	  ctts->table = (quicktime_ctts_table_t*)realloc(ctts->table, sizeof(quicktime_ctts_table_t) * ctts->entries_allocated);
  }
...
void quicktime_update_ctts(quicktime_ctts_t *ctts, int sample, int duration)
  ...
  if(sample >= ctts->entries_allocated)
    ...
    ctts->table = (quicktime_ctts_table_t*)realloc(ctts->table, ctts->entries_allocated * sizeof(*(ctts->table)));
    }
