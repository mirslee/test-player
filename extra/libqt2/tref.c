...
static quicktime_track_reference_t * add_reference(quicktime_tref_t *tref)
  {
  tref->num_references++;
  tref->references = (quicktime_track_reference_t*)realloc(tref->references,
...
static void add_track(quicktime_track_reference_t * ref,
  ...
  {
  ref->num_tracks++;
  ref->tracks = (uint32_t*)realloc(ref->tracks,
...
int quicktime_read_tref(quicktime_t *file, quicktime_tref_t *tref,
  ...
  while(quicktime_position(file) < tref_atom->end)
    ...
    ref->num_tracks = (int)((atom.end - quicktime_position(file)) / 4);
    ref->tracks = (uint32_t*)calloc(ref->num_tracks, sizeof(*ref->tracks));
