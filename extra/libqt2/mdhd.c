...
void quicktime_read_mdhd(quicktime_t *file, quicktime_mdhd_t *mdhd)
{
	...
	{
		mdhd->creation_time = quicktime_read_uint32(file);
		mdhd->modification_time = quicktime_read_uint32(file);
	}
	mdhd->time_scale = quicktime_read_int32(file);
	if (mdhd->version == 1) 
		mdhd->duration = quicktime_read_int64(file);
	else
		mdhd->duration = quicktime_read_uint32(file);
...
void quicktime_write_mdhd(quicktime_t *file, quicktime_mdhd_t *mdhd)
{
	...
	int version = mdhd->duration<INT32_MAX ? 0 : 1;
	int timescale = mdhd->time_scale;
	__int64 duration = mdhd->duration;
	quicktime_write_char(file, version);
	quicktime_write_int24(file, mdhd->flags);
	if(version==1)
	{
		quicktime_write_int64(file, mdhd->creation_time);
		quicktime_write_int64(file, mdhd->modification_time);
		quicktime_write_int32(file, timescale);
		quicktime_write_int64(file, duration);
	}
	else
	{
		quicktime_write_int32(file, (int)mdhd->creation_time);
		quicktime_write_int32(file, (int)mdhd->modification_time);
		quicktime_write_int32(file, timescale);
		quicktime_write_int32(file, (int)duration);
	}
