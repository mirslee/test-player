...
void lqt_add_timecode_track(quicktime_t * file, int track,
  ...
  vm->timecodes = (uint32_t*)malloc(framerate * sizeof(*vm->timecodes));
  vm->timecodes_alloc = framerate;
...
static uint32_t * cache_timecodes(quicktime_t * file,
                                  quicktime_trak_t * trak, int64_t * total)
  {
  uint32_t * ret;
  int i, j;
  int num_read = 0, num;
  ...
  *total = quicktime_track_samples(file, trak);
  ret = (uint32_t*)malloc((size_t)(*total * sizeof(*ret)));
  ...
  if(!force)
    ...
    if(vm->timecodes_written)
      ...
      quicktime_update_stts(stts, vm->timecodes_written-1,
                            (long)(time - vm->timecode_timestamp));
  ...
  if(vm->num_timecodes &&
     ((vm->num_timecodes >= vm->timecodes_alloc) || force))
    {
		...
		for(i = 0; i < vm->num_timecodes; i++)
 		...
 		quicktime_write_chunk_footer(file, vm->timecode_track, vm->current_timecode_chunk,&chunk_atom, vm->num_timecodes);
		quicktime_write_pad(file, 512);
