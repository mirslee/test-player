...
void quicktime_iods_add_track(quicktime_iods_t * iods, quicktime_trak_t * trak)
  {
	  iods->tracks = (quicktime_iods_t::track*)realloc(iods->tracks, sizeof(*iods->tracks)*(iods->num_tracks+1));
...
void quicktime_read_iods(quicktime_t *file, quicktime_iods_t * iods)
  ...
  iods->tracks = (quicktime_iods_t::track*)calloc(iods->num_tracks, sizeof(*(iods->tracks)));
