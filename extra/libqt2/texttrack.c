...
int lqt_read_text(quicktime_t * file, int track, char ** text, int * text_alloc,
  ...
  int sd_id = 1;
  file_position = quicktime_sample_to_offset(file, trak, &sd_id,(int)ttrack->current_position);
  ...
  if(string_length)
    ...
    if(ttrack->text_buffer_alloc < string_length)
      ...
      ttrack->text_buffer = (char*)realloc(ttrack->text_buffer, ttrack->text_buffer_alloc);
      }
    ...
    if(ttrack->cnv)
      ...
      if(*text_alloc < string_length)
        ...
        *text = (char*)realloc(*text, *text_alloc);
...
int lqt_add_text_track(quicktime_t * file, int timescale)
  ...
  file->ttracks =
    (quicktime_text_map_t*)realloc(file->ttracks, (file->total_ttracks+1)*sizeof(quicktime_text_map_t));
...
int lqt_write_text(quicktime_t * file, int track, const char * text,
  ...
  quicktime_write_chunk_footer(file, trak, (int)ttrack->current_chunk,
                               &chunk_atom, 1);

  quicktime_update_stts(&trak->mdia.minf.stbl.stts, (int)ttrack->current_position, (int)duration);
...
void lqt_set_text_box(quicktime_t * file, int track,
  ...
  if(quicktime_match_32(stsd->format, "text"))
    ...
    trak->tkhd.track_width  = (float)(right - left);
    trak->tkhd.track_height = (float)(bottom - top);
    }
