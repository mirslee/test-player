...
static WORD MACEtab1[] = { 0xfff3, 0x0008, 0x004c, 0x00de, 
0x00de, 0x004c, 0x0008, 0xfff3 };

static WORD MACEtab3[] = { 0xffee, 0x008c, 0x008c, 0xffee };

static WORD MACEtab2[][8] = {
...
typedef struct
{
	MACEState left_state;
	MACEState right_state;
} Param;


/* /// "chomp3()" */
static void chomp3(MACEState *s,BYTE** outPtr,ULONG step,BYTE val,WORD tab1[],WORD tab2[][8])
{
	short current = tab2[(s->index & 0x7f0) >> 4][val];
	if (current+s->lev > 32767) current=32767;
	else if (current+s->lev < -32768) current=-32767;
	else current+=s->lev;
	s->lev=current-(current >> 3);
	**outPtr = (current >> 8);
	*outPtr += step;
	if ( ( s->index += tab1[val]-(s->index>>5) ) < 0 ) s->index = 0;
}

/* /// "chomp6()" */
static void chomp6(MACEState*	s,BYTE** outPtr,ULONG step,BYTE val,WORD tab1[],WORD tab2[][8])
{
	short current = tab2[(s->index & 0x7f0) >> 4][val];

	if ((s->previous^current)>=0) {
		if (s->factor+506>32767) s->factor=32767;
		else s->factor+=506;
	} else {
		if (s->factor-314<-32768) s->factor=-32767;
		else s->factor-=314;
	}

	if (current+s->level>32767) current=32767;
	else if (current+s->level<-32768) current=-32767;
	else current+=s->level;

	s->level=((current*s->factor) >> 15);
	current>>=1;

	**outPtr=(s->previous+s->prev2-((s->prev2-current) >> 2)) >> 8;
	*outPtr+=step;

	**outPtr=(s->previous+current+((s->prev2-current) >> 2)) >> 8;
	*outPtr+=step;
...
static void Exp1to3(MACEState* state,const BYTE *inBuffer,BYTE* outBuffer,ULONG cnt,ULONG numChannels,ULONG whichChannel)
{
	BYTE pkt;
	BYTE* outPtr = outBuffer+(whichChannel-1);

	state->index=state->lev=0;

	inBuffer+=(whichChannel-1)*2;

	while (cnt>0) {
		pkt=inBuffer[0];
		chomp3(state, &outPtr, numChannels,  pkt       & 7, MACEtab1, MACEtab2);
		chomp3(state, &outPtr, numChannels, (pkt >> 3) & 3, MACEtab3, MACEtab4);
		chomp3(state, &outPtr, numChannels,  pkt >> 5     , MACEtab1, MACEtab2);
		pkt=inBuffer[1];
		chomp3(state, &outPtr, numChannels,  pkt       & 7, MACEtab1, MACEtab2);
		chomp3(state, &outPtr, numChannels, (pkt >> 3) & 3, MACEtab3, MACEtab4);
		chomp3(state, &outPtr, numChannels,  pkt >> 5     , MACEtab1, MACEtab2);
...
static void Exp1to6(MACEState* state,const BYTE *inBuffer,BYTE* outBuffer,ULONG cnt,ULONG numChannels,ULONG whichChannel)
{
	BYTE* outPtr;
	BYTE pkt;

	state->previous=state->prev2=state->index=state->level=state->factor=0;

	inBuffer+=(whichChannel-1);
	outPtr= outBuffer+(whichChannel-1);

	while (cnt>0) {
		pkt=*inBuffer;

		chomp6(state, &outPtr, numChannels,  pkt >> 5     , MACEtab1, MACEtab2);
		chomp6(state, &outPtr, numChannels, (pkt >> 3) & 3, MACEtab3, MACEtab4);
		chomp6(state, &outPtr, numChannels,  pkt       & 7, MACEtab1, MACEtab2);
...
int decode_mace6(void* codec,WAVEFORMATEX* wfx,BYTE *input,ULONG inputsize,BYTE  *output,ULONG outsize)
{
	...
	if (channels==1) {
		Exp1to6(&p->left_state, input,output, inputsize,1,1);
	} else if (channels==2) {
		Exp1to6(&p->left_state, input,output, inputsize/2,2,1);
		Exp1to6(&p->right_state, input,output, inputsize/2,2,2);
	} else {
		fprintf(stderr, "decode_mace6: unsupported number of channels (%d)\n", channels);
	}
	return inputsize*6;
}
...
int decode_mace3(void* codec,WAVEFORMATEX* wfx,BYTE *input,ULONG inputsize,BYTE  *output,ULONG outsize)
{
	...
	int channels = wfx->nChannels;
	if (channels==1) {
		Exp1to3(&p->left_state, input,output, inputsize/2,1,1);
	} else if (channels==2) {
		Exp1to3(&p->left_state, input,output, inputsize/4,2,1);
		Exp1to3(&p->right_state, input,output, inputsize/4,2,2);
	} else {
		fprintf(stderr, "decode_mace3: unsupported number of channels (%d)\n", channels);
	}
	return inputsize*3;
}
...
void* init_mace3(WAVEFORMATEX* wfx)
{
	...
	memset(&p->left_state, 0, sizeof(MACEState));
	memset(&p->right_state, 0, sizeof(MACEState));
	return p;
}

void delete_mace3(void* codec)
{
	Param *p = (Param*)codec;
	if (p) free(p);
}
