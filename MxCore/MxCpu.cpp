...
static bool mxCpuheck (const char *name, void (*func) (void))
{
    pid_t pid = fork();
...
MX_SSE static void SSE_test (void)
{
    asm volatile ("xorps %%xmm0,%%xmm0\n" : : : "xmm0", "xmm1");
}
#endif
#if defined (CAN_COMPILE_3DNOW)
MX_MMX static void ThreeD_Now_test (void)
{
...
#else /* _WIN32 || __OS2__ */
# define mxCpuCheck(name, func) (1)
#endif
#endif

/**
 * Determines the CPU capabilities and stores them in cpu_flags.
 * The result can be retrieved with MX_CPU().
 */
void mxCpuInit()
{
...
# if !defined (__MMX__)
    if( ! (i_edx & 0x00800000) )
        goto out;
# endif
    i_capabilities |= MX_CPU_MMX;
    
    if( i_edx & 0x02000000 )
        i_capabilities |= MX_CPU_MMXEXT;
# if defined (CAN_COMPILE_SSE) && !defined (__SSE__)
    if (( i_edx & 0x02000000 ) && mxCpuCheck ("SSE", SSE_test))
# endif
    {
        /*if( i_edx & 0x02000000 )*/
        i_capabilities |= MX_CPU_SSE;
        if (i_edx & 0x04000000)
            i_capabilities |= MX_CPU_SSE2;
        if (i_ecx & 0x00000001)
            i_capabilities |= MX_CPU_SSE3;
        if (i_ecx & 0x00000200)
            i_capabilities |= MX_CPU_SSSE3;
        if (i_ecx & 0x00080000)
            i_capabilities |= MX_CPU_SSE4_1;
        if (i_ecx & 0x00100000)
            i_capabilities |= MX_CPU_SSE4_2;
    }
...
# if defined (CAN_COMPILE_3DNOW) && !defined (__3dNOW__)
    if ((i_edx & 0x80000000) && mxCpuCheck ("3D Now!", ThreeD_Now_test))
# endif
        i_capabilities |= MX_CPU_3dNOW;
...
#   endif
    ...
    if( i_error == 0 && i_has_altivec != 0 )
        i_capabilities |= MX_CPU_ALTIVEC;
    
#   elif defined( CAN_COMPILE_ALTIVEC )
    if (mxCpuCheck ("Altivec", Altivec_test))
        i_capabilities |= MX_CPU_ALTIVEC;
...
# ifdef __ANDROID__
    if (android_getCpuFeatures() & ANDROID_CPU_ARM_FEATURE_NEON)
        i_capabilities |= MX_CPU_ARM_NEON;
# endif
...
unsigned mxCpu()
{
    ...
    pthread_once (&once, mxCpuInit);
#endif
    return cpu_flags;
}
#endif

/*void MX_CPU_dump (vlc_object_t *obj)
{
