...
ncx_slab_init(ncx_slab_pool_t *pool)
{
    ...
    if (ncx_slab_max_size == 0) {
        ...
        ncx_slab_exact_size = ncx_pagesize / (8 * sizeof(vxuintptr));
        for (n = ncx_slab_exact_size; n >>= 1; ncx_slab_exact_shift++) {
            /* void */
        }
    }

	VXTHREAD_MUTEX_INIT(&pool->mutex);
	 ...
	 * slots[n].slab 是一个vxuintptr类型，在32位系统，与unsigned等价，即有32bit可用
	...
	pool->pages->prev = (vxuintptr)&pool->free;
	 ...
	 */
    pool->start = (u_char *)
		ncx_align_ptr((vxuintptr)p + pages * sizeof(ncx_slab_page_t),
                                 ncx_pagesize);
...
ncx_slab_alloc_locked(ncx_slab_pool_t *pool, size_t size)
{
    ...
    if (page->next != page) {
        ...
        if (shift < ncx_slab_exact_shift) {
            ...
            do {
				 ...
				 */
                p = (page - pool->pages) << ncx_pagesize_shift;
				bitmap = (vxuintptr *)(pool->start + p);

				/*
				 * (1 << (ncx_pagesize_shift - shift)) 算出一个page存放的obj数目
				 *  / (sizeof(vxuintptr) * 8) 计算需要占用多少vxuintptr 来存储bitmap
				 *
				 *  即map 表示占用多少个vxuintptr（32bit系统占4字节，64bit占8字节）
				 */
                ...
                for (n = 0; n < map; n++) {
                    ...
                    if (bitmap[n] != NCX_SLAB_BUSY) {
                        ...
                        for (m = 1, i = 0; m; m <<= 1, i++) {
							 ...
							 *  可拆分为 (1<<shift) * ((n*sizeof(vxuintptr)*8) + i) 理解
							 *  ((n*sizeof(vxuintptr)*8) + i) 表示第n个obj
							 *  (1<<shift) 为一个obj的大小
							 */
							i = ((n * sizeof(vxuintptr) * 8) << shift)
                                + (i << shift);
            ...
            do {
                if ((page->slab & NCX_SLAB_MAP_MASK) != mask) {
					...
					for (m = (vxuintptr)1 << NCX_SLAB_MAP_SHIFT, i = 0;
                         m & mask;
                         m <<= 1, i++)
                    {
                        if ((page->slab & m)) {
    ...
    if (page) {
        if (shift < ncx_slab_exact_shift) {
			...
			/*
			 * map为bitmap所覆盖的vxuintptr数
			 */
			map = (1 << (ncx_pagesize_shift - shift)) / (sizeof(vxuintptr) * 8);
			 ...
			 * (vxuintptr) &slots[slot] | NCX_SLAB_SMALL，在prev字段里保存了slab的规模(small,exact,big,page四类)
			 * 这样做的好处主要是简化了free的逻辑;在free函数再详细讨论
			 */
            page->slab = shift;
            page->next = &slots[slot];
			page->prev = (vxuintptr)&slots[slot] | NCX_SLAB_SMALL;
            ...
            page->slab = 1;
            page->next = &slots[slot];
			page->prev = (vxuintptr)&slots[slot] | NCX_SLAB_EXACT;
			...
			page->slab = ((vxuintptr)1 << NCX_SLAB_MAP_SHIFT) | shift;
            page->next = &slots[slot];
			page->prev = (vxuintptr)&slots[slot] | NCX_SLAB_BIG;
...
ncx_slab_free_locked(ncx_slab_pool_t *pool, void *p)
{
    ...
    case NCX_SLAB_SMALL:
		 ...
		 * 3.算出obj所在的是哪块bitmap(一个vxuintptr为一块bitmap)
		 * 4.算出bitmap的首地址(即第一块bitmap的地址)
		 */
		n = ((vxuintptr)p & (ncx_pagesize - 1)) >> shift;
		m = (vxuintptr)1 << (n & (sizeof(vxuintptr) * 8 - 1));
		n /= (sizeof(vxuintptr) * 8);
		bitmap = (vxuintptr *)((vxuintptr)p & ~(ncx_pagesize - 1));
        ...
        if (bitmap[n] & m) {
            ...
            if (page->next == NULL) {
				...
				page->prev = (vxuintptr)&slots[slot] | NCX_SLAB_SMALL;
				page->next->prev = (vxuintptr)page | NCX_SLAB_SMALL;
            }
    ...
    case NCX_SLAB_EXACT:
		 ...
		 */
		m = (vxuintptr)1 <<
			(((vxuintptr)p & (ncx_pagesize - 1)) >> ncx_slab_exact_shift);
        ...
        if (slab & m) {
            if (slab == NCX_SLAB_BUSY) {
				...
				page->prev = (vxuintptr)&slots[slot] | NCX_SLAB_EXACT;
				page->next->prev = (vxuintptr)page | NCX_SLAB_EXACT;
            }
    ...
    case NCX_SLAB_BIG:
		...
		/*
		 * (((vxuintptr) p & (ncx_pagesize - 1)) >> shift) 算出对应bitmap哪个bit
		 */
		m = (vxuintptr)1 << ((((vxuintptr)p & (ncx_pagesize - 1)) >> shift)
                              + NCX_SLAB_MAP_SHIFT);
        ...
        if (slab & m) {
            ...
            if (page->next == NULL) {
				...
				page->prev = (vxuintptr)&slots[slot] | NCX_SLAB_BIG;
				page->next->prev = (vxuintptr)page | NCX_SLAB_BIG;
            }
    ...
    for (page = pool->free.next; page != &pool->free; page = page->next) {
        ...
        if (page->slab >= pages) {
            ...
            if (page->slab > pages) {
                ...
                p->next = &page[pages];
				page->next->prev = (vxuintptr)&page[pages];
    ...
    ncx_uint_t pages)
{
	 ...
	 * 把回收的page重新放到free链表
	 */
	page->prev = (vxuintptr)&pool->free;
    page->next = pool->free.next;

	page->next->prev = (vxuintptr)page;
		...
		if (ncx_slab_empty(pool, prev)) {
			for (; prev >= pool->pages; prev--) {
				if (prev->slab != 0) 
				{
					...
					page->next->prev = (vxuintptr)&pool->free;
	...
	for (i = 0; i < stat->pages; i++)
	{
		...
		switch (type) {
			...
			case NCX_SLAB_EXACT:
				...
				if (slab == NCX_SLAB_BUSY) {
					stat->used_size += sizeof(vxuintptr) * 8 * ncx_slab_exact_size;
					stat->b_exact += sizeof(vxuintptr) * 8 * ncx_slab_exact_size;
				}
				else {
					for (m = 1; m; m <<= 1) {
						if (slab & m) {
			...
			case NCX_SLAB_BIG:
				...
				for (m = (vxuintptr)1 << NCX_SLAB_MAP_SHIFT; m & mask; m <<= 1)
				{
					if ((page->slab & m)) {
						stat->used_size += obj_size;
						stat->b_big     += obj_size;
...
#ifdef _DEBUG
int LOG_LEVEL = LV_INFO;
#else
int LOG_LEVEL = LV_ERROR;
#endif

ncx_slab_pool ncx_create_pool(int pool_size)
{
	u_char* space = (u_char *)_vxmalloc(pool_size);
	ncx_slab_pool sp = (ncx_slab_pool_t*)space;
...
void ncx_destroy_pool(ncx_slab_pool pool)
{
	if (pool)
	{
		VXTHREAD_MUTEX_DESTROY(&pool->mutex);
		_vxfree(pool);
	}
}

class CVxDefaultSlabPool
{
public:
	CVxDefaultSlabPool()
	{
		pool = ncx_create_pool(0x8000000);
	}
	~CVxDefaultSlabPool()
	{
		ncx_slab_stat_t stat = { 0 };
		ncx_slab_stat(pool, &stat);
		ncx_destroy_pool(pool);
	}
	ncx_slab_pool pool;
}g_slabpool;
...
{
	void *p = ncx_slab_alloc(g_slabpool.pool, size);
	if (p == 0)
	{
...
{
	ncx_slab_free(g_slabpool.pool, p);
}

VX_EXT_API void slab_stat(ncx_slab_stat_t *stat)
{
	ncx_slab_stat(g_slabpool.pool, stat);
}
