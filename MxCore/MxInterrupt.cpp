...
{
    vlc_interrupt_t *ctx = (vlc_interrupt_t*)malloc(sizeof (*ctx));
    if (likely(ctx != NULL))
...
{
    vlc_interrupt_finish((vlc_interrupt_t*)opaque);
}
...
static void vlc_interrupt_forward_wake(void *opaque)
{
    ...
    vlc_interrupt_t *to = (vlc_interrupt_t*)data[0];
    vlc_interrupt_t *from = (vlc_interrupt_t*)data[1];
...
static int vlc_poll_i11e_inner( pollfd */*restrict*/ fds, unsigned nfds,
